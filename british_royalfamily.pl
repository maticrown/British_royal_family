...
% loading the facts, relations and other relevant code
...
:- consult('/proj/relations/eldest.pl').
...
run :-
    ...
    (
        (
            FirstWord = 'stop', !, write_ln('Stopped successfully.'), fail
        ) ;
        (
            (
                FirstWord = 'l', !, make, write_ln('Reloaded successfully.') ;
